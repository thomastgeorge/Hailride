...
const AdminRequests = ({ requestType }) => {
    ...
    useEffect(() => {
        ...
        Axios.post("/api/v1/service/getAllRequestsByType", {
    ...
    const handleAccept = () => {
        if (window.confirm("Do you want to accept this request?") === true) {
            Axios.put('/api/v1/service/acceptRequestById', {
                requestId: requestId,
    ...
    return (
        <div className='p-2'>
                                ...
                                </div>
                                {
                                    requestType === "project" ?
                                        <div className='d-flex'>
                                            <h2 className='mt-2'><b>{req?.appName?.charAt(0).toUpperCase() + req?.appName?.slice(1)}</b></h2>
                                            {/* <div className='d-flex align-items-center mx-1 px-2 py-1 bg-dark rounded-3 text-white'><p className='m-0 mx-2'>Project</p></div> */}
                                        </div>
                                        :
                                        <div className='d-flex'>
                                            <h2 className='mt-2'><b>{req?.subject?.charAt(0).toUpperCase() + req?.subject?.slice(1)}</b></h2>
                                            {/* <div className='d-flex align-items-center mx-1 px-2 py-1 bg-dark rounded-3 text-white'><p className='m-0 mx-2'>Project</p></div> */}
                                        </div>
            ...
            <Modal title={<b># {requestId}</b>} open={isModalOpen} onOk={handleOk} onCancel={handleCancel}
                footer={[
                    <Button key="back" onClick={handleAccept}>
                        Accept
                    </Button>,
